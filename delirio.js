(function(){
  ...
  function scalePage(){
    ...
    /*
      Escritorio: ancho editorial de 1000px.
      Móvil: el CSS convierte la página a 100% real;
      NO se reduce con transform:scale().
    */
  ...
  */
  ...
  /* ============================================================
     IMÁGENES DE PERFIL
     El usuario puede colocar estos archivos dentro de /imagenes/.
     ============================================================ */

  document.querySelectorAll(".note[data-profile]").forEach(card => {
    if(card.querySelector(".note-profile")) return;
    const src=(card.dataset.profile||"").trim();
    if(!src) return;
    const img=document.createElement("img");
    img.className="note-profile";
    img.src=src;
    img.alt="";
    img.loading="lazy";
  ...
  document.querySelectorAll(".blog-link-card[data-profile]").forEach(card => {
    if(card.querySelector(".blog-card-image")) return;
    const src=(card.dataset.profile||"").trim();
    if(!src) return;
    const img=document.createElement("img");
  ...
  document.querySelectorAll(".index-entry[data-profile]").forEach(card => {
    const slot=card.querySelector(".index-entry-profile");
    if(!slot || slot.querySelector("img")) return;
    const src=(card.dataset.profile||"").trim();
    if(!src) return;
    const img=document.createElement("img");
    img.src=src;
    img.className="index-entry-profile-image";
    img.alt="";
    img.loading="lazy";
    img.onerror=()=>{};
  ...
  /* ============================================================
     COMPARTIR
     data-share-url="" = usa la URL actual.
     Si quieres fijar una URL concreta, escribe la URL dentro del
     atributo data-share-url del botón de esa página.
     ============================================================ */

  document.querySelectorAll(".share-button").forEach(button=>{
    button.addEventListener("click", async ()=>{
      const url=button.dataset.shareUrl?.trim() || window.location.href;
